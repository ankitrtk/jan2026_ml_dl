...
try calculating the fractions:
f1 = c1 / (c1 + c2)
f2 = c2 / (c1 + c2)

Now, You need to design a function which gives the following:
f1  | f2  | purity  | impurity (1 - purity)
1   | 0   | 1       | 0
0   | 1   | 1       | 0
0.5 | 0.5 | 0       | 1
0.1 | 0.9 | .8       | .2
...
purity = (f1 - f2)**2 or abs(f1 - f2) 
impurity: 1 - (f1 - f2)**2 -> 1 - (f1+f2)**2 + 4*f1*f2 -> 4*f1* f2
ratio:
  # pick the smaller fraction and put it in numerator
  0/1 -> 0
  .5/.5 -> 1
  0.1 / .9 -> ..
  0.01 / .99 -> .01
